...
const ProjectCard = (project: ProjectItem) => {
  return (
    <div className="md:p-8 p-4 h-auto min-h-[300px] bg-stone-100 z-10 border border-black/10 rounded-xl relative">
      <div className="flex ">
        <img
          src={project.icon}
          className={`${project.extraStyling} ${project.bgColor} drop-shadow-sm p-4 rounded-lg md:w-30 md:h-30 w-25 h-25`}
        />
        <div className="flex flex-col ml-5 justify-end">
          <h4 className="text-xl md:text-3xl font-normal text-left">{project.title}</h4>
          <p className="text-gray-700 text-base md:text-lg font-bold text-left">{project.tools}</p>
        </div>
      </div>
